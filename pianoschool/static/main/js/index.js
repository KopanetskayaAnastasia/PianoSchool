...
// --- Ear Training Piano Recording Logic ---
...
function startRecording() {
    ...
    // UI: показать индикатор записи, если нужно
}

function stopRecording() {
    isRecording = false;
    // UI: скрыть индикатор записи, если нужно
    // Возвращаем только последовательность нот (без времени)
    return recordedNotes.map(x => x.note);
}

// Модификация playSound:
function playSound(keyElem) {
    let soundId = keyElem.dataset.sound;
    let sound = document.getElementById(soundId);
    if (sound) {
...
// Проигрывание эталонной последовательности
function playSequence(sequence, delay=500) {
    let i = 0;
    function playNext() {
        if (i < sequence.length) {
            let keyElem = document.querySelector('.key.' + sequence[i]);
            if (keyElem) playSound(keyElem);
            setTimeout(() => {
                if (keyElem) stopSound(keyElem);
                i++;
                playNext();
            }, delay);
        }
